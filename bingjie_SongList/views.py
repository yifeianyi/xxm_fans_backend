...
from .models import bingjie_Songs, bingjie_site_setting
import json
import random
...
def song_list(request):
    if request.method == 'GET':
        ...
        # 构建查询
        songs = bingjie_Songs.objects.all()
...
def get_languages(request):
    """获取所有语言列表"""
    if request.method == 'GET':
        languages = bingjie_Songs.objects.exclude(language='').values_list('language', flat=True).distinct()
...
def get_styles(request):
    """获取所有曲风列表"""
    if request.method == 'GET':
        styles = bingjie_Songs.objects.exclude(style='').values_list('style', flat=True).distinct()
...
def site_settings(request):
    if request.method == 'GET':
        settings = bingjie_site_setting.objects.all().values()
        # 简化photoURL，只返回文件名
        updated_settings = []
...
def favicon(request):
    ...
    try:
        setting = bingjie_site_setting.objects.get(position=1)
        # 重定向到对应的图片文件，前端统一使用/photos/前缀
        from django.shortcuts import redirect
        filename = setting.photoURL
        if '/' in filename:
            filename = filename.split('/')[-1]
        photo_url = 'bingjie_SongList_frontend/photos/' + filename
        return redirect(photo_url)
    except bingjie_site_setting.DoesNotExist:
