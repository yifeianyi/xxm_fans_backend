...
class FansDIYBilibiliImporter(BilibiliImporter):
    ...
    def import_bv_work(self, bvid, collection_name, notes=""):
        ...
        try:
            ...
            # 下载并保存封面
            final_cover_url = self.download_and_save_cover(cover_url, collection_name, pubdate)
    ...
    def download_and_save_cover(self, cover_url, collection_name, pubdate):
        """下载并保存封面图片"""
        if not cover_url:
            return None
...
def download_and_save_cover(cover_url, collection_name, pubdate):
    """下载并保存封面图片"""
    importer = FansDIYBilibiliImporter()
    return importer.download_and_save_cover(cover_url, collection_name, pubdate) 